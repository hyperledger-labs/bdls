...
func (c *BFTChain) TestMultiClients() {
...
func (c *BFTChain) TestOrderClient1(wg *sync.WaitGroup) {
	...
	for i := 0; i < 1; i++ {
		/*env := &common.Envelope{
			Payload: protoutil.MarshalOrPanic(&common.Payload{
				...
				Data:   []byte(fmt.Sprintf("100 Byte Asset Size-TEST_MESSAGE-UNCC-Client-1_MESSAGE-UNCC-Client-1-Msg number: %v.", i)),
				//Data: []byte("100 Byte Asset Size-TEST_MESSAGE-UNCC-Client-1_MESSAGE-UNCC-Client-1-Msg number: ."),
			}),
		}*/
...
func (c *BFTChain) TestOrderClient2(wg *sync.WaitGroup) {
...
// This test will run after 20 second for network healthchck after TCP IO error being generated
func (c *BFTChain) TestOrderClient3(wg *sync.WaitGroup) {
	//time.Sleep(1000 * time.Millisecond)
...
// This test will run after 20 second for network healthchck after TCP IO error being generated
func (c *BFTChain) TestOrderClient4(wg *sync.WaitGroup) {
	//time.Sleep(1000 * time.Millisecond)
	c.Logger.Infof("For client %v", 3)
	for i := 0; i < 2500; i++ {
		env := &common.Envelope{
